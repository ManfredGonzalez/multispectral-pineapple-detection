...
def train(opt, use_seed):
    ...
    for num_exp in range(1,opt.experiments+1):
        ...
        if opt.use_only_vl:
            opt.saved_path = opt.saved_path + f'/{num_exp}_{params.project_name}/'
            opt.log_path = opt.log_path + f'/{num_exp}_{params.project_name}/tensorboard/'
        else:
            opt.saved_path = opt.saved_path + f'/{num_exp}_{params.project_name}_{opt.bands_to_apply}/'
            opt.log_path = opt.log_path + f'/{num_exp}_{params.project_name}_{opt.bands_to_apply}/tensorboard/'
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=4, verbose=True, factor=0.5, min_lr=1e-6)
