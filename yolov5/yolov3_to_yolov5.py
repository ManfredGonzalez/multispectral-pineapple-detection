...
def split_data(file_input_dir, output_folder, 
    ...
    if not os.path.exists(output_folder + name_1 + "/"):
        ...
        os.mkdir(output_folder + name_1 + "/images/")
        os.mkdir(output_folder + name_1 + "/labels/")
    if not os.path.exists(output_folder + name_2 + "/"):
        os.mkdir(output_folder + name_2 + "/")
        os.mkdir(output_folder + name_2 + "/images/")
        os.mkdir(output_folder + name_2 + "/labels/")
    if not os.path.exists(output_folder + name_3 + "/") and (set_3 != 0.0):
        os.mkdir(output_folder + name_3 + "/")
        os.mkdir(output_folder + name_3 + "/images/")
        os.mkdir(output_folder + name_3 + "/labels/")
    #--------------------------------------------------------------------------
    ...
    for img_index, img_name in enumerate(X_train):    
        ...
        copyfile(file_input_dir + img_name + '.' + img_extension, folder + 'images/' + img_name + '.' + img_extension)
        copyfile(file_input_dir + img_name + '.txt', folder + 'labels/' + img_name + '.txt')
        if multispectral:
            for bandName in multispectral:
                copyfile(file_input_dir + img_name + '_' + bandName + '.TIF', folder + 'images/' + img_name + '_' + bandName + '.TIF')
    #-------------------
...
def create_project_file(output_project_file, train_path, test_path, obj_list):
    with open(output_project_file, 'w') as my_file:
        my_file.write(f'train: {train_path}\n')
        my_file.write(f'val: {test_path}\n')
        my_file.write(f'nc: {len(obj_list)}\n')
        my_file.write(f'names: {obj_list}\n')
...
def get_args():
    ...
    parser.add_argument('--output_path', type=str, default='datasets/yolo_format/apple_yolov4pytorch/')
    ...
    parser.add_argument('--multispectral', type=str, default=None) #Set this to None in case you want just the visible light images
...
if __name__ == '__main__':
    ...
    output_folder = opt.output_path + '/' + opt.project_name + '/'
    output_yml =  opt.output_path + '/' + opt.project_name + '/data.yaml'
